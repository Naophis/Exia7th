  ...
  int32_T arg_mode, int32_T arg_time_step, t_ego *arg_next_ego)
{
  ...
  if (arg_tgt->v_max >= 0.0F) {
    rtb_Product2 = arg_ego->v * arg_ego->v - arg_tgt->end_v * arg_tgt->end_v;
    if (arg_ego->state == 2.0F || std::abs(rtb_Product2) /
      ...
      if (arg_ego->v > arg_tgt->end_v) {
        if (mpc_tgt_calc_P.ManualSwitch_CurrentSetting_i == 1) {
          ...
          rtb_Switch_e = std::abs(rtb_Product2 / (mpc_tgt_calc_P.Gain_Gain_n *
  ...
  rtb_Merge2 = rtb_Abs7 * static_cast<real32_T>(arg_time_step) *
    mpc_tgt_calc_P.Gain1_Gain_e;
  arg_next_ego->dist = arg_ego->dist + rtb_Merge2;
  if (arg_mode == 1) {
    if (arg_ego->sla_param.state != 0) {
      rtb_Switch_e = mpc_tgt_calc_P.Constant_Value_j;
    } else {
      rtb_Product2 = static_cast<real32_T>(arg_time_step) + static_cast<real32_T>
        (arg_ego->sla_param.counter);
      if (rtb_Product2 >= arg_ego->sla_param.limit_time_count) {
        rtb_Switch_e = mpc_tgt_calc_P.Constant1_Value;
      } else {
        rtb_Product2 = rtb_Product2 * mpc_tgt_calc_P.Gain_Gain_e /
          arg_ego->sla_param.base_time - mpc_tgt_calc_P.Constant_Value_ne;
        rtb_Power = rt_powf_snf(rtb_Product2, arg_ego->sla_param.pow_n -
          mpc_tgt_calc_P.Constant1_Value_mn);
        rtb_Product2 *= rtb_Power;
        ...
        rtb_Switch_e = mpc_tgt_calc_P.Gain1_Gain_l * arg_ego->sla_param.pow_n *
          rtb_Power / (rtb_Product2 * rtb_Product2) * std::exp(rtb_Gain2_h) /
    ...
    if (arg_ego->sla_param.counter >= arg_ego->sla_param.limit_time_count) {
      rtb_Power = static_cast<real32_T>(mpc_tgt_calc_P.Constant1_Value_d);
    } else {
      rtb_Power = static_cast<real32_T>(mpc_tgt_calc_P.Gain_Gain * rtb_Switch_e *
        static_cast<real32_T>(arg_time_step)) + arg_ego->w;
    }

    arg_next_ego->alpha = static_cast<real32_T>(rtb_Switch_e);
    rtb_Product2 = std::fmod(static_cast<real32_T>(arg_ego->sla_param.counter) +
      static_cast<real32_T>(arg_time_step), 4.2949673E+9F);
    arg_next_ego->sla_param.counter = rtb_Product2 < 0.0F ? -static_cast<int32_T>
      (static_cast<uint32_T>(-rtb_Product2)) : static_cast<int32_T>(static_cast<
    ...
    if (arg_ego->pivot_state == 2.0F || std::abs(arg_ego->w * arg_ego->w -
         ...
         abs(rtb_Gain2_h)) + rtb_Product2 >= rtb_Power) {
      boolean_T rtb_RelationalOperator_a;
      if (rtb_Gain2_h > mpc_tgt_calc_P.Switch2_Threshold) {
      ...
      if (rtb_RelationalOperator_a) {
        if (mpc_tgt_calc_P.ManualSwitch_CurrentSetting != 1) {
          if (rtb_Gain2_h > mpc_tgt_calc_P.Switch1_Threshold) {
            rtb_Switch_e = rtb_Power - rtb_Product2;
            if (mpc_tgt_calc_P.Constant1_Value_l >= rtb_Switch_e || rtIsNaN
                (rtb_Switch_e)) {
              rtb_Switch_e = mpc_tgt_calc_P.Constant1_Value_l;
            }

            rtb_Gain2_h = static_cast<real32_T>(std::abs((arg_ego->w *
        ...
        rtb_Gain2_h = mpc_tgt_calc_P.Constant_Value_ph;
      }
      ...
      rtb_Gain2_h = static_cast<real32_T>(rtb_Switch_e);
    } else {
      rtb_Gain2_h = static_cast<real32_T>(mpc_tgt_calc_P.Constant_Value_n);
      rtb_Merge1_p = mpc_tgt_calc_P.Constant2_Value_g;
    }

    if (rtb_Product2 < rtb_Power) {
      rtb_Power = mpc_tgt_calc_P.Gain_Gain_d * rtb_Gain2_h *
        static_cast<real32_T>(arg_time_step) + arg_ego->w;
      arg_next_ego->alpha = rtb_Gain2_h;
      arg_next_ego->sla_param.counter = arg_ego->sla_param.counter;
      arg_next_ego->pivot_state = rtb_Merge1_p;
    } else {
      rtb_Power = mpc_tgt_calc_P.Constant_Value_pr;
    ...
    int32_T rtb_Merge1_p;
    rtb_Power = std::abs(arg_ego->img_ang);
    rtb_Gain2_h = mpc_tgt_calc_P.Gain2_Gain_i * arg_tgt->alpha;
    rtb_Switch_ju = std::abs(arg_tgt->tgt_angle);
    rtb_Product2 = arg_ego->w * arg_ego->w - arg_tgt->end_w * arg_tgt->end_w;
    if (arg_ego->pivot_state == 2.0F || std::abs(rtb_Product2) /
        (mpc_tgt_calc_P.Gain1_Gain_hy * std::abs(rtb_Gain2_h)) + rtb_Power >=
        mpc_tgt_calc_P.Gain_Gain_m * rtb_Switch_ju || rtb_Power >=
        mpc_tgt_calc_P.Gain4_Gain * rtb_Switch_ju) {
      boolean_T rtb_RelationalOperator_a;
      if (rtb_Gain2_h > mpc_tgt_calc_P.Switch2_Threshold_m) {
        rtb_RelationalOperator_a = arg_ego->w < arg_tgt->end_w;
      } else {
        rtb_RelationalOperator_a = arg_ego->w > arg_tgt->end_w;
      }
      ...
      if (rtb_RelationalOperator_a) {
        if (mpc_tgt_calc_P.ManualSwitch_CurrentSetting_j != 1) {
          if (rtb_Gain2_h > mpc_tgt_calc_P.Switch1_Threshold_i) {
            rtb_Switch_e = rtb_Switch_ju - rtb_Power;
            if (mpc_tgt_calc_P.Constant1_Value_m >= rtb_Switch_e || rtIsNaN
                (rtb_Switch_e)) {
              rtb_Switch_e = mpc_tgt_calc_P.Constant1_Value_m;
            }

            rtb_Gain2_h = static_cast<real32_T>(std::abs(rtb_Product2 /
        ...
        rtb_Gain2_h = mpc_tgt_calc_P.Constant_Value_h;
      }

      rtb_Merge1_p = mpc_tgt_calc_P.Constant2_Value_f2;
    } else if (arg_ego->pivot_state == 0.0F && rtb_Power <
               mpc_tgt_calc_P.Gain5_Gain * rtb_Switch_ju) {
      rtb_Gain2_h = arg_tgt->alpha;
      rtb_Merge1_p = mpc_tgt_calc_P.Constant1_Value_oj;
    } else {
      rtb_Gain2_h = static_cast<real32_T>(mpc_tgt_calc_P.Constant_Value_p);
      rtb_Merge1_p = mpc_tgt_calc_P.Constant2_Value_j;
    }

    if (rtb_Power < rtb_Switch_ju) {
      rtb_Power = mpc_tgt_calc_P.Gain_Gain_c * rtb_Gain2_h *
        static_cast<real32_T>(arg_time_step) + arg_ego->w;
      arg_next_ego->alpha = rtb_Gain2_h;
      arg_next_ego->sla_param.counter = arg_ego->sla_param.counter;
      arg_next_ego->pivot_state = rtb_Merge1_p;
    } else {
      rtb_Power = mpc_tgt_calc_P.Constant_Value_m;
    ...
    arg_next_ego->sla_param.counter = arg_ego->sla_param.counter;
    arg_next_ego->pivot_state = arg_ego->pivot_state;
    rtb_Power = mpc_tgt_calc_P.Constant_Value_e;
  ...
  rtb_Gain2_h = rtb_Power * static_cast<real32_T>(arg_time_step) *
    mpc_tgt_calc_P.Gain1_Gain_g;
  arg_next_ego->ang = arg_ego->ang + rtb_Gain2_h;
  arg_next_ego->img_dist = arg_ego->img_dist + rtb_Merge2;
  rtb_Gain2_h += arg_ego->img_ang;
  if (rtb_Abs7 != 0.0F) {
    rtb_Merge2 = (mpc_tgt_calc_P.Constant_Value_g / static_cast<real32_T>
    ...
    rtb_Merge2 = mpc_tgt_calc_P.Constant3_Value_a;
  }

  rtb_Switch_ju = rtb_Gain2_h + rtb_Merge2;
  rtb_Product_b = rtb_Abs7 * std::cos(rtb_Switch_ju) *
    mpc_tgt_calc_P.Gain3_Gain_m * static_cast<real32_T>(arg_time_step) +
    arg_ego->slip_point.x;
  rtb_Merge_f = rtb_Abs7 * std::sin(rtb_Switch_ju) * mpc_tgt_calc_P.Gain4_Gain_o
    * static_cast<real32_T>(arg_time_step) + arg_ego->slip_point.y;
  rtb_Product_h = rtb_Abs7 * std::cos(rtb_Gain2_h) *
    mpc_tgt_calc_P.Gain1_Gain_cq * static_cast<real32_T>(arg_time_step) +
    arg_ego->ideal_point.x;
  rtb_Merge_l = rtb_Abs7 * std::sin(rtb_Gain2_h) * mpc_tgt_calc_P.Gain2_Gain_c *
    static_cast<real32_T>(arg_time_step) + arg_ego->ideal_point.y;
  if (rtb_Abs6 > 0.0F) {
    real32_T rtb_Add_p;
    rtb_Add_p = arg_ego->cnt_delay_accl_ratio + mpc_tgt_calc_P.Constant_Value_hn;
    arg_next_ego->cnt_delay_accl_ratio = rtb_Add_p;
    if (arg_tgt->limit_accl_ratio_cnt != 0.0F) {
      rtb_Product2 = arg_tgt->limit_accl_ratio_cnt;
    } else {
      rtb_Product2 = mpc_tgt_calc_P.Constant1_Value_i;
    }

    rtb_Product2 = rtb_Add_p / rtb_Product2;
    if (rtb_Product2 > mpc_tgt_calc_P.Saturation_UpperSat) {
      rtb_Product2 = mpc_tgt_calc_P.Saturation_UpperSat;
    } else if (rtb_Product2 < mpc_tgt_calc_P.Saturation_LowerSat) {
      rtb_Product2 = mpc_tgt_calc_P.Saturation_LowerSat;
    }

    arg_next_ego->delay_accl = rtb_Abs6 * rtb_Product2;
    arg_next_ego->cnt_delay_decel_ratio = arg_ego->cnt_delay_decel_ratio;
  } else if (rtb_Abs6 < 0.0F) {
    real32_T rtb_Add_p;
    rtb_Add_p = arg_ego->cnt_delay_decel_ratio +
      mpc_tgt_calc_P.Constant_Value_my;
    arg_next_ego->cnt_delay_decel_ratio = rtb_Add_p;
    if (arg_tgt->limit_decel_ratio_cnt != 0.0F) {
      rtb_Product2 = arg_tgt->limit_decel_ratio_cnt;
    } else {
      rtb_Product2 = mpc_tgt_calc_P.Constant1_Value_op;
    }

    rtb_Product2 = rtb_Add_p / rtb_Product2;
    if (rtb_Product2 > mpc_tgt_calc_P.Saturation_UpperSat_i) {
      rtb_Product2 = mpc_tgt_calc_P.Saturation_UpperSat_i;
    } else if (rtb_Product2 < mpc_tgt_calc_P.Saturation_LowerSat_e) {
      rtb_Product2 = mpc_tgt_calc_P.Saturation_LowerSat_e;
    }
  ...
  arg_next_ego->w = rtb_Power;
  arg_next_ego->img_ang = rtb_Gain2_h;
  ...
  arg_next_ego->ideal_point.x = rtb_Product_h;
  arg_next_ego->ideal_point.y = rtb_Merge_l;
  ...
  arg_next_ego->slip_point.x = rtb_Product_b;
  arg_next_ego->slip_point.y = rtb_Merge_f;
  arg_next_ego->slip_point.theta = rtb_Switch_ju;
  arg_next_ego->slip_point.v = arg_ego->slip_point.v;
  arg_next_ego->slip_point.w = arg_ego->slip_point.w;
  arg_next_ego->slip_point.slip_angle = rtb_Merge2;
  arg_next_ego->kanayama_point = arg_ego->kanayama_point;
  arg_next_ego->trj_diff.x = rtb_Product_h - rtb_Product_b;
  arg_next_ego->trj_diff.y = rtb_Merge_l - rtb_Merge_f;
  arg_next_ego->trj_diff.theta = mpc_tgt_calc_P.Gain5_Gain_e * rtb_Gain2_h -
    mpc_tgt_calc_P.Gain6_Gain * rtb_Switch_ju;
